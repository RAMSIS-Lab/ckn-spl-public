...
class DNN_CKN(torch.nn.Module):
    def __init__(self, x_dim:int=2, y_dim:int=2):
        ...
        self.in_shape1 = spdlayers.in_shape_from(self.x_dim)
        self.in_shape2 = spdlayers.in_shape_from(self.y_dim)
        ...
        self.l2 = nn.Sequential(
            nn.Linear(self.gru_hidden_dim, H2),
            nn.ReLU(),
            nn.Linear(H2, self.in_shape1),
            spdlayers.Cholesky(output_shape=self.x_dim,
        ...
        self.l4 = nn.Sequential(
            nn.Linear(self.gru_hidden_dim, H2),
            nn.ReLU(),
            nn.Linear(H2, self.in_shape2),
            spdlayers.Cholesky(output_shape=self.y_dim,
                          symmetry='anisotropic',
                          positive='Abs',
                          min_value=1e-8)
        )
    ...
    def forward(self, state_inno, observation_inno, diff_state, diff_obs, linearization_error, Jacobian):
        ...
        Pk = torch.squeeze(Pk, dim=0)
        Sk = torch.squeeze(Sk, dim=0)
        
        Sk = torch.inverse(Sk) + 1e-8 * torch.eye(Sk.size(0))     
