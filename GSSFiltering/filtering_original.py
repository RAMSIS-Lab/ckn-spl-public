from GSSFiltering.dnn import DNN_KalmanNet_GSS, DNN_SKalmanNet_GSS, KNet_architecture_v2, DNN_SKN_Sym
...
class KalmanNet_Filter():
    ...
    def filtering(self, observation):
        ...
        cov_post = P_posterior.clone().detach()
        ...
        self.obs_past = observation.detach().clone()
        self.state_post = x_post.detach().clone()
        self.cov_post = cov_post.clone().detach()
...
class Cholesky_KalmanNet_Filter():
    def __init__(self, GSSModel:GSSModel):
        ...
        self.kf_net = DNN_SKN_Sym(self.x_dim, self.y_dim)
        self.init_state = GSSModel.init_state
        self.reset(clean_history=True)
