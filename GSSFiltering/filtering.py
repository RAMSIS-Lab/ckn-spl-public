from GSSFiltering.dnn import DNN_KalmanNet_GSS, DNN_SKalmanNet_GSS, KNet_architecture_v2, DNN_CKN
...
class Extended_Kalman_Filter():
    ...
    def __init__(self, GSSModel:GSSModel):
        ...
        self.cov_history = self.init_cov.detach().clone()
    ...
    def reset(self, clean_history=False):
        ...
        if clean_history:
            # Clean history for a new Trajectory, with inital states
            self.state_history = self.init_state.detach().clone()
            self.cov_history = torch.unsqueeze(self.init_cov, dim=0)
            # self.cov_trace_history = torch.zeros((1,))
        # self.state_history = torch.cat((self.state_history, self.state_post), axis=1)
        # self.cov_history = torch.cat((self.cov_history, torch.unsqueeze(self.cov_post, dim=0)), axis=0)
    ...
    def filtering(self, observation):
        with torch.no_grad():
            ...
            self.state_history = torch.cat((self.state_history, x_post), axis=1)    
            self.cov_history = torch.cat((self.cov_history, torch.unsqueeze(cov_post, dim=0)), axis=0)    
...
class KalmanNet_Filter():
    ...
    def reset(self, clean_history=False):
        ...
        if clean_history:
            ...
            self.cov_history = self.init_cov.detach().clone()    
            self.cov_trace_history = torch.zeros((1,))      
        # self.state_history = torch.cat((self.state_history, self.state_post), 1)

        # if self.cov_history.dim() != 3:
        #     self.cov_history = torch.unsqueeze(self.cov_history, dim=0)

        # self.cov_history = torch.cat((self.cov_history, torch.unsqueeze(self.cov_post, dim=0)))  
    ...
    def filtering(self, observation):
        ...
        try:
            H_jacob = self.GSSModel.Jacobian_g(x_predict)
            H_tilde = torch.inverse(H_jacob.T @ H_jacob)

            P_posterior = (torch.eye(self.x_dim) - K_gain @ H_jacob) @ H_tilde  @ H_jacob.T @ torch.inverse(torch.eye(self.y_dim) - H_jacob @ K_gain) @ H_jacob @ K_gain @ self.R @ H_jacob @ H_tilde

        except:
            P_posterior = torch.zeros_like(torch.unsqueeze(cov_post, dim=0))
            print("Impossible Uncertainty Analysis with KalmanNet")
        
        cov_post = P_posterior.clone()
        self.cov_post = cov_post.detach().clone()
        
        if self.cov_history.dim() != 3:
            self.cov_history = torch.unsqueeze(self.cov_history, dim=0)
        
        self.cov_history = torch.cat([self.cov_history, torch.unsqueeze(cov_post, dim=0)], 0)
...
class Split_KalmanNet_Filter():
    def __init__(self, GSSModel:GSSModel):
        ...
        self.Pk_history = torch.eye(self.x_dim)
        self.Sk_history = torch.eye(self.y_dim)
    ...
    def reset(self, clean_history=False):
        ...
        if clean_history:
            ...
            self.state_history = self.init_state.detach().clone()
            self.cov_history = torch.unsqueeze(self.init_cov, dim=0)
            self.Pk_history = torch.unsqueeze(torch.eye(self.x_dim), dim=0)
            self.Sk_history = torch.unsqueeze(torch.eye(self.y_dim), dim=0)
        # self.state_history = torch.cat((self.state_history, self.state_post), axis=1)
    ...
    def filtering(self, observation):
        ...
        cov_post = (torch.eye(self.x_dim) - K_gain @ H_jacob) @ Pk

        self.cov_post = cov_post.detach().clone()
        self.cov_history = torch.cat((self.cov_history, torch.unsqueeze(cov_post, dim=0)), axis=0) 

        self.Pk_history = torch.cat([self.Pk_history, torch.unsqueeze(Pk, dim=0)], 0)
        self.Sk_history = torch.cat([self.Sk_history, torch.unsqueeze(Sk, dim=0)], 0)


class Cholesky_KalmanNet_Filter():
    def __init__(self, GSSModel:GSSModel):
        ...
        self.kf_net = DNN_CKN(self.x_dim, self.y_dim)
    ...
    def filtering(self, observation):
        ...
        # # diff_obs_in = diff_obs
        # (Pk, Sk) = self.kf_net(state_inno_in, residual_in, diff_state_in, diff_obs_in, linearization_error, H_jacob.reshape((-1,1)))
        # Pk = Pk + self.Q + 1e-6 * torch.eye(Pk.size(0))
        # Sk = Sk + self.R + 1e-6 * torch.eye(Sk.size(0))

        # K_gain = Pk @ torch.transpose(H_jacob, 0, 1) @ Sk
        # print(Sk)
        try:
            K_gain = Pk @ torch.transpose(H_jacob, 0, 1) @ torch.inverse(Sk)
        except:
            Sk = Sk + 1e-6 * torch.eye(Sk.size(0))    
            K_gain = Pk @ torch.transpose(H_jacob, 0, 1) @ torch.inverse(Sk)
        ...
        # cov_post = (torch.eye(self.x_dim) - K_gain @ H_jacob) @ Pk
        cov_post = (torch.eye(self.x_dim) - K_gain @ H_jacob) @ Pk * (torch.eye(self.x_dim) - K_gain @ H_jacob).T + K_gain @ self.R @ K_gain.T
